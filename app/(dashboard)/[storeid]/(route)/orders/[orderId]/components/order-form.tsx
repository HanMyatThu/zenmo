...
export const OrderForm: React.FC<{
  ...
  const variants = JSON.parse(initialData.variants) as string[];
  const quantity = JSON.parse(initialData.quantity) as string[];

  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.storeId}/orders/${initialData.id}`);
      router.refresh();
